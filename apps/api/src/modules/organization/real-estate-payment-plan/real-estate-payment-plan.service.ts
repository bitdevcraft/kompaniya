...
import { orgRealEstatePaymentPlansTable } from '@repo/database/schema';
...
export class RealEstatePaymentPlanService {
  ...
  async getDataTable(
    ...
    return await this.paginationRepositoryService.getPaginatedDataTable({
      ...
      cacheKey: `${Keys.RealEstatePaymentPlan.paginated(userId, organizationId)}-${JSON.stringify(query)}`,
      query,
      organizationId,
    });
  }
