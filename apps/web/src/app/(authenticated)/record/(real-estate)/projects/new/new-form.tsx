...
import {
  ...
  projectRecordSchema,
  ProjectRecordSubmitValues,
} from "../r/[id]/project-record-schema";
...
const useSubmit = () => {
  return useMutation({
    mutationFn: async (payload: ProjectRecordSubmitValues) => {
...
export function NewRecordForm({ onFinish }: NewRecordFormProps) {
  ...
  const onSubmit = (values: ProjectRecordFormValues) => {
    const payload = extractCreateValues(values, layout);

    submit.mutate(payload as ProjectRecordSubmitValues, {
      onSuccess: () => {
