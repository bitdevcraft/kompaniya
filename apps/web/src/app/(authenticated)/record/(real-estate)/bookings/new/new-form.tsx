...
import {
  ...
  bookingRecordSchema,
  BookingRecordSubmitValues,
} from "../r/[id]/booking-record-schema";
...
const useSubmit = () => {
  return useMutation({
    mutationFn: async (payload: BookingRecordSubmitValues) => {
...
export function NewRecordForm({ onFinish }: NewRecordFormProps) {
  ...
  const onSubmit = (values: BookingRecordFormValues) => {
    const payload = extractCreateValues(values, layout);

    submit.mutate(payload as BookingRecordSubmitValues, {
      onSuccess: () => {
