...
const useSubmit = () => {
  return useMutation({
    mutationFn: async (payload: OpportunityRecordSubmitValues) => {
...
export function NewRecordForm({ onFinish }: NewRecordFormProps) {
  ...
  const form = useForm<OpportunityRecordFormValues>({
    ...
    defaultValues: createDefaultValuesForLayout(opportunityRecordLayout),
  });

  const submit = useSubmit();

  const onSubmit = (values: OpportunityRecordFormValues) => {
    const payload = extractCreateValues(values, opportunityRecordLayout);

    submit.mutate(payload as OpportunityRecordSubmitValues, {
      onSuccess: () => {
