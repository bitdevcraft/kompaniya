...
export function useDataTableColumns(
  setRowAction: React.Dispatch<
    React.SetStateAction<DataTableRowAction<OrgOpportunity> | null>
  >,
) {
  const onDelete = makeRowAction(setRowAction, "delete");
  const onUpdate = makeRowAction(setRowAction, "update");

  const columns: ColumnDef<OrgOpportunity>[] = [
    {
      id: "select",
      header: ({ table }) => (
        <Checkbox
  ...
  return getTableColumns<OrgOpportunity>({ setRowAction, columns });
}
