...
export function OrgDataTable(props: OrgDataTableProps) {
  ...
  const { data } = useQuery({
    queryKey: [
      `opportunities-${activeOrganization?.id}`,
      props.search.page,
    ...
    queryFn: async () => {
      const response = await axios.get<{
        ...
        `${env.NEXT_PUBLIC_BASE_SERVER_URL}/api/organization/opportunity/paginated${qs ? "?" : ""}${qs}`,
        {
  ...
  const [_rowAction, setRowAction] =
    React.useState<DataTableRowAction<OrgOpportunity> | null>(null);
