...
export function OrgDataTable(props: OrgDataTableProps) {
  ...
  const { data } = useQuery({
    queryKey: [
      `real-estate-properties-${activeOrganization?.id}`,
      props.search.page,
    ...
    queryFn: async () => {
      const response = await axios.get<{
        data: OrgRealEstateProperties[];
        pageCount: number;
      }>(
        `${env.NEXT_PUBLIC_BASE_SERVER_URL}/api/organization/real-estate-property/paginated${qs ? "?" : ""}${qs}`,
        {
  ...
  const [_rowAction, setRowAction] =
    React.useState<DataTableRowAction<OrgRealEstateProperties> | null>(null);
