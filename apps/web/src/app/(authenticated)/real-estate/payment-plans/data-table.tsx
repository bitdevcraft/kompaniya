...
export function OrgDataTable(props: OrgDataTableProps) {
  ...
  const { data } = useQuery({
    queryKey: [
      `real-estate-payment-plans-${activeOrganization?.id}`,
      props.search.page,
    ...
    queryFn: async () => {
      const response = await axios.get<{
        data: OrgRealEstatePaymentPlan[];
        pageCount: number;
      }>(
        `${env.NEXT_PUBLIC_BASE_SERVER_URL}/api/organization/real-estate-payment-plan/paginated${qs ? "?" : ""}${qs}`,
        {
  ...
  const [_rowAction, setRowAction] =
    React.useState<DataTableRowAction<OrgRealEstatePaymentPlan> | null>(null);
