...
export function OrgDataTable(props: OrgDataTableProps) {
  ...
  const { data } = useQuery({
    queryKey: [
      `email-campaigns-${activeOrganization?.id}`,
      props.search.page,
    ...
    queryFn: async () => {
      const response = await axios.get<{
        data: OrgEmailCampaign[];
        pageCount: number;
      }>(
        `${env.NEXT_PUBLIC_BASE_SERVER_URL}/api/organization/email-campaign/paginated${qs ? "?" : ""}${qs}`,
        {
  ...
  const [_rowAction, setRowAction] =
    React.useState<DataTableRowAction<OrgEmailCampaign> | null>(null);
